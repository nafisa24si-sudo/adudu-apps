          ...
          {menuItems.map((item, index) => (
            ...
            >
              <Link to="/" className="flex items-center">
                <item.icon
                  className={`mr-4 text-xl transition-all duration-300 group-hover:scale-110 ${activeMenu === item.name ? item.color : "text-gray-400 group-hover:text-green-500"}`}
                />
                <span>{item.name}</span>
              </Link>

              {activeMenu === item.name && (
                <Link to="/" className="absolute left-0 top-0 h-full w-1 bg-green-500 animate-slideInRight">
                </Link>
              )}
